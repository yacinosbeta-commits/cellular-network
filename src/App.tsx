...
// --- Mock Data Generator ---
// Since Web APIs cannot access raw cellular hardware (CID, LAC, etc.),
// we simulate the data to demonstrate the UI and logic as requested.
const generateMockData = (): NetworkData => {
  ...
  return {
    ...
    mcc: '603', // Example MCC for Algeria
    mnc: '01',  // Example MNC
...
export default function App() {
  ...
  const refreshData = useCallback(() => {
    ...
    // Simulate a slight delay for the "real-time" feel
    setTimeout(() => {
  ...
  const handleRequestPermission = () => {
    // In a real Android app, this would trigger the native permission dialog.
    // In a web app, we simulate the flow.
      ...
      <div className="min-h-screen bg-black text-zinc-100 flex items-center justify-center p-6 font-sans">
        ...
        >
          ...
          <p className="text-zinc-600 text-xs">
            Note: This is a web demonstration. Real hardware access requires a native Android application.
          </p>
        </motion.div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 font-sans selection:bg-emerald-500/30">
      {/* Header */}
      <header className="sticky top-0 z-10 bg-black/80 backdrop-blur-md border-bottom border-zinc-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-emerald-500 rounded-xl flex items-center justify-center">
            <Radio className="text-black" size={24} />
          </div>
          <div>
            <h1 className="font-bold text-lg leading-tight">NetMonitor</h1>
            <div className="flex items-center gap-2">
              <span className="flex h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Live Tracking</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        <section className="bg-gradient-to-br from-zinc-900 to-black border border-zinc-800 p-6 rounded-3xl space-y-4">
          <div className="flex justify-between items-start">
            ...
            <div className="bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20">
              <span className="text-emerald-500 text-xs font-bold">{data.technology}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-4">
            <div className="space-y-1">
              ...
              <div className="flex items-end gap-2">
                <span className="text-2xl font-mono font-bold">{data.rssi}</span>
        ...
        <section className="bg-zinc-900/30 border border-zinc-800/50 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-zinc-500 leading-relaxed">
              This dashboard provides a real-time visualization of cellular telemetry. 
              Values like RSRP (Reference Signal Received Power) and RSRQ (Reference Signal Received Quality) 
              are critical for determining handover performance and data throughput in LTE/5G networks.
            </p>
          </div>
        </section>
        ...
        <div className="text-center space-y-2 pt-4">
          ...
          <div className="flex justify-center gap-4">
            <div className="flex items-center gap-1 text-zinc-500">
              <Smartphone size={12} />
              <span className="text-[10px]">Android 14 Compatible</span>
            </div>
            <div className="flex items-center gap-1 text-zinc-500">
              <Wifi size={12} />
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-zinc-800 text-white px-6 py-3 rounded-2xl shadow-2xl border border-zinc-700 flex items-center gap-2 z-50"
          >
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-xl border-t border-zinc-800 px-8 py-4 flex justify-around items-center">
